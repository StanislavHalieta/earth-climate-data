...
def format_gmsl_indicator_data(data_list):
    ...
    v_recent = (h_last - (data_list[-window]["gmsl"] * 10.0)) / (t_last - data_list[-window]["decimal_year"])
    ...
    return {
        ...
        "multi_source_coefficients": {
            ...
            "potential_energy_debt": "HIGH" if v_recent < 0 and observed_rise > 50 else "STABLE"
        },
...
def hazard_index_final(raw, v_recent):
    ...
    return raw if v_recent >= 0 else raw * 1.2 # +20% до небезпеки за "приховану енергію"