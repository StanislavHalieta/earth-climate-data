...
from app.api.noaa.ocean_pentad_heat.ocean_pentad_heat_data import (
    parse_ocean_pentad_heat_data,
)
from app.api.noaa.relative_sea_level.relative_sea_level_parser import (
    parse_relative_sea_level_data,
)
from app.api.noaa.noaa_ice_extent.noaa_ice_data_parser import parse_noaa_ice_data
from app.api.noaa.paleo_sea_level.noaa_paleo_parser import (
    parse_noaa_paleo_sea_level_data,
)
from app.api.noaa.relative_sea_level_summary.relative_sea_level_summary_parser import (
...
def sync_noaa_data():
    ...
    raw_vostok_dust = create_noaa_session(
...
def sync_noaa_data():
    ...
    raw_methane_brw = create_noaa_session(
        base_url=base_url_gml, endpoint=endpoint_methane_brw
    )
    ...
    raw_methane_mlo = create_noaa_session(
        base_url=base_url_gml, endpoint=endpoint_methane_mlo
    )
    ...
    solar_flux_data = create_noaa_session(
        base_url=base_url_services, endpoint=endpoint_solar
    )
    db.save_metric("noaa_solar_flux", solar_flux_data)

    # Sunspot
    endpoint_sunspot = os.getenv("NOAA_SUNSPOT")
    sunspot_data = create_noaa_session(
        base_url=base_url_services, endpoint=endpoint_sunspot
    )
    db.save_metric("noaa_sunspot", sunspot_data)

    # KP Index
    endpoint_kp = os.getenv("NOAA_KP_INDEX")
    kp_index_data = create_noaa_session(
        base_url=base_url_services, endpoint=endpoint_kp
    )
