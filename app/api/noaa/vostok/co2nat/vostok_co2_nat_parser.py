...
def parse_vostok_co2_data(raw_data):
    ...
    try:
        ...
        if not lines:
            return {
                "status": "error",
                "message": "Файл порожній або містить лише коментарі",
            }
        ...
        if "gas_ageBP" not in df.columns or "CO2" not in df.columns:
            return {
                "status": "error",
                "message": f"Не знайдено колонки 'gas_ageBP' або 'CO2'. Наявні: {list(df.columns)}",
            }
            ...
            result_records.append(
                {
                    "calendar_year": int(
                        2026 - int(age_bp)
                    ),  # Розраховуємо абсолютний рік від 2026-го
