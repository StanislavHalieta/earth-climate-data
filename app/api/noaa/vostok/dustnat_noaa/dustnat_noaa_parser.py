...
def parse_vostok_dust_data(raw_data):
    ...
    try:
        ...
        if not lines:
            return {
                "status": "error",
                "message": "Файл порожній або містить лише метадані",
            }
        ...
        if "ice_ageBP" not in df.columns or "dust_ppm" not in df.columns:
            return {
                "status": "error",
                "message": f"Не знайдено колонки 'ice_ageBP' або 'dust_ppm'. Наявні: {list(df.columns)}",
            }
            ...
            result_records.append(
                {
                    "calendar_year": int(
                        2026 - int(ice_age)
                    ),  # Перерахунок в абсолютну шкалу часу від 2026 року
                    "ice_age_bp": int(ice_age),
                    "dust_concentration_ppm": (
                        float(dust_val) if dust_val is not None else None
                    ),
                }
            )
