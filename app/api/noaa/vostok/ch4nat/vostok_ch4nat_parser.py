...
def parse_vostok_ch4_data(raw_text):
    ...
    try:
        ...
        if not lines:
            return {"status": "error", "message": "Файл не містить корисних даних (тільки коментарі або порожній)"}
        ...
        if 'gas_ageBP' not in df.columns or 'CH4' not in df.columns:
            return {"status": "error", "message": f"Не знайдено колонки 'gas_ageBP' або 'CH4'. Наявні: {list(df.columns)}"}
        ...
        for _, row in df.iterrows():
            ...
            result_records.append({
                "calendar_year": int(2026 - int(age_bp)), # Абсолютний календарний рік
