...
def parse_vostok_recent_temp(raw_data):
    ...
    try:
        ...
        if not lines:
            return {
                "status": "error",
                "message": "Файл не містить корисних даних або порожній",
            }
        ...
        if "age_CE" not in df.columns or "tempanom" not in df.columns:
            return {
                "status": "error",
                "message": f"Не знайдено колонки 'age_CE' або 'tempanom'. Наявні: {list(df.columns)}",
            }
            ...
            result_records.append(
                {
                    ...
                    "temperature_anomaly_celsius": (
                        float(temp_anom) if temp_anom is not None else None
                    ),
                }
            )
        ...
        return {
            "status": "error",
            "message": f"Помилка парсингу температурних даних: {str(e)}",
        }
