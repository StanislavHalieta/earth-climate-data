...
def parse_relative_sea_level_summary_data(raw_text):
    ...
    try:
        ...
        if not lines:
            return {
                "status": "error",
                "message": "Файл не містить копії корисних даних",
            }
            ...
            result_records.append(
                {
                    ...
                    "calendar_year": int(
                        2026 - int(row["age_mean"])
                    ),  # Розрахунок відносного року для індексу
                    "age_mean_cal_bp": int(row["age_mean"]),
                    "age_range_2sigma": [
                        int(row["age_younger_limit"]),
                        int(row["age_older_limit"]),
                    ],
                    "classification": str(row["classification"]).strip(),
                    "rsl_meters": float(row["rsl"]) if row["rsl"] is not None else None,
                    "limit_elev_meters": (
                        float(row["limit_elev"])
                        if row["limit_elev"] is not None
                        else None
                    ),
                    "errors": {
                        "up": (
                            float(row["elev_err_up"])
                            if row["elev_err_up"] is not None
                            else None
                        ),
                        "down": (
                            float(row["elev_err_down"])
                            if row["elev_err_down"] is not None
                            else None
                        ),
                    },
                }
