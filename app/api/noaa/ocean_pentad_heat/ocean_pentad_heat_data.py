...
def parse_ocean_pentad_heat_data(binary_content, depth_layer="0-2000"):
    ...
    try:
        if not binary_content or not isinstance(binary_content, bytes):
            return {"status": "error", "message": "Очікувався бінарний контент (bytes) NetCDF файлу"}

        # 🛠️ КЛЮЧОВИЙ МОМЕНТ: Відкриваємо БЕЗ автоматичного декодування часу, 
        # щоб xarray не падав через кастомні календарі NOAA
        with xr.open_dataset(io.BytesIO(binary_content), engine='h5netcdf', decode_times=False) as ds:
            
            # Автоматично шукаємо наукову змінну, що відповідає за аномалію тепла
            hc_var_name = [v for v in ds.data_vars if 'hc' in v or 'heat' in v or 'anom' in v]
            if not hc_var_name:
            ...
            spatial_dims = [dim for dim in ['lat', 'lon', 'latitude', 'longitude'] if dim in ds[var_to_extract].dims]
            ...
            if 'time' in global_trend.coords:
                ...
                try:
                    ...
                    base_date = pd.Timestamp("1955-01-01") # залізобетонний фолбек для NOAA OHC
                
                # Проходимо по масиву
                for i in range(len(time_values)):
                    # Витягуємо чисте число через .item() або індекс, щоб уникнути помилки скалярів
                    try:
                        raw_val = anomaly_values[i].item() if hasattr(anomaly_values[i], 'item') else float(anomaly_values[i])
                        raw_time = time_values[i].item() if hasattr(time_values[i], 'item') else int(time_values[i])
                    except Exception:
        ...
        return {"status": "error", "message": f"Помилка обробки NetCDF файлу {depth_layer}: {str(e)}"}