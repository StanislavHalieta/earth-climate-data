...
def create_noaa_session(base_url=None, endpoint=None, hemisphere=None):
    ...
    try:
        ...
        # 2. Сценарій: Передано півкулю (N або S) — це 100% щоденний лід на NSIDC
            ...
            if hemi not in ["N", "S"]:
                raise ValueError(
                    f"Невалідний параметр півкулі: '{hemisphere}'. Очікується 'N' або 'S'."
                )
        ...
        # 3. Сценарій: Передано відносний шлях — йдемо на головний сервер NOAA
            ...
            raise ValueError(
                "Необхідно передати або параметр 'hemisphere', або 'endpoint'."
            )

        # Захист від порожніх конфігів у .env
        if not base_url or not current_endpoint:
            raise ValueError(
                f"URL не сформовано. base_url: {base_url}, endpoint: {current_endpoint}"
            )
        ...
        logger.info(
            f"✅ Запит до NOAA: {base_url.strip('/')}/{current_endpoint.lstrip('/')}"
        )
        return response
