def parse_relative_sea_level_data(raw_text):
    ...
    try:
        ...
        while line_idx < len(lines):
            ...
            if len(parts) >= 5 and parts[0].isdigit() and '.' in parts[1] and '.' in parts[2]:
                ...
                for _ in range(data_rows_count):
                    ...
                    if len(data_parts) >= 4:
                        ...
                        station_data["records"].append({
                            "calendar_year": int(2026 - age_bp), # Перевод у шкалу нашого часу
                            "age_bp": age_bp,
                            "error": error_val,
        ...
        return {"status": "error", "message": f"Помилка ієрархічного парсингу: {str(e)}"}