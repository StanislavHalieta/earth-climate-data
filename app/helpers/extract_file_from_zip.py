...
def extract_file_from_zip_parser(
    zip_binary_content, target_filename=None, as_bytes=False
):
    try:
        ...
        with zipfile.ZipFile(zip_stream) as archive:
            ...
            if target_filename:
                if target_filename not in file_list:
                    logger.error(
                        f"❌ Файл '{target_filename}' не знайдено в архіві. Доступні: {file_list}"
                    )
                    return None
                ...
                logger.info(
                    f"📦 Автоматично обрано перший файл з архіву: {file_to_extract}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"⚠️ Не вдалося декодувати {file_to_extract} в UTF-8. Пробуємо latin-1..."
                )
